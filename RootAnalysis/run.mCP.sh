#!/bin/bash
echo hello
#proc="MilliQ.UFO.14TeV"

#masses=(0.10 0.28 0.43 0.60 0.78 1.00 1.25 1.52 1.84 2.20 2.60 3.04 3.54 4.10 4.71 5.40 6.15 6.98 7.90 8.90 10.00 11.20 12.50 14.00 15.50 17.20 19.10 21.10 23.30 25.60 28.20 30.90 33.90 37.10 40.50 44.20 48.20 52.50 57.10 62.10 67.40 73.00 79.10 85.60 92.60 100.00)

masses=(10.0) #0.1 1.0 10.0 100.0)
#charges=(0.001 0.0012 0.0014 0.0016 0.0018 0.002 0.0022 0.0024 0.0026 0.0028 0.003 0.0032 0.0034 0.0036 0.0038 0.004 0.005 0.006)
#charges=(0.007 0.008 0.009 0.01 0.02)
charges=(0.0016) #(0.03 0.05 0.07 0.09 0.1)
nEv=10000

#risetime=(0.00 0.08 0.16 0.24 0.32 0.40 0.48 0.56 0.64 0.72 0.80 0.88 0.96 1.04 1.12 1.20 1.28 1.36 1.44 1.52 1.60 1.68 1.76 1.84 1.92 2.00)

for charge in ${charges[*]}
do
for mass in ${masses[*]}
do
	echo Now submitting charge $charge
        sbatch -p cerberus --ntasks=1 ./submit.mcp.exclusions.sh $charge $mass $nEv
done
done
